---
name: ipen
description: Autonomous novel writing CLI agent with web workbench (iPen Studio) - use for creative fiction writing, novel generation, style imitation, chapter continuation/import, EPUB export, AIGC detection, and fan fiction. Native English support with 10 built-in English genre profiles (LitRPG, Progression Fantasy, Isekai, Cultivation, System Apocalypse, Dungeon Core, Romantasy, Sci-Fi, Tower Climber, Cozy Fantasy). Also supports Chinese web novel genres (xuanhuan, xianxia, urban, horror, other). Multi-agent pipeline with foundation reviewer, hook seed excerpt recovery, audit-driven repair loop, review reject state rollback, 33-dimension auditing, token usage analytics, creative brief input, structured logging (JSON Lines), multi-model routing, custom OpenAI-compatible provider support, and ipen Studio web UI for visual book management, chapter review, real-time writing progress, market radar, and analytics.
version: 2.3.0
metadata: { "openclaw": { "emoji": "📖", "requires": { "bins": ["ipen", "node"], "env": [] }, "primaryEnv": "", "homepage": "https://github.com/Narcooo/ipen", "install": [{ "id": "npm", "kind": "node", "package": "@actalk/ipen", "label": "Install iPen (npm)" }] } }
---

# iPen - Autonomous Novel Writing Agent

iPen is a CLI tool for autonomous fiction writing powered by LLM agents. It orchestrates a multi-agent pipeline (Radar → Planner → Composer → Architect → Writer → Observer → Reflector → Normalizer → Auditor → Reviser) to generate, audit, and revise novel content with zero human intervention per chapter.
...
ipen init my-writing-project

# Configure your LLM provider (OpenAI, Anthropic, or any OpenAI-compatible API)
ipen config set-global --provider openai --base-url https://api.openai.com/v1 --api-key sk-xxx --model gpt-4o
# For compatible/proxy endpoints, use --provider custom:
# ipen config set-global --provider custom --base-url https://your-proxy.com/v1 --api-key sk-xxx --model gpt-4o
```

### Multi-Model Routing (Optional)
```bash
# Assign different models to different agents — balance quality and cost
ipen config set-model writer claude-sonnet-4-20250514 --provider anthropic --base-url https://api.anthropic.com --api-key-env ANTHROPIC_API_KEY
ipen config set-model auditor gpt-4o --provider openai
...
1. **Initialize and create book**:
   ```bash
   ipen book create --title "My Novel Title" --genre xuanhuan --chapter-words 3000
   # Or with a creative brief (your worldbuilding doc / ideas):
   ipen book create --title "My Novel Title" --genre xuanhuan --chapter-words 3000 --brief my-ideas.md
   ```
...
2. **Generate initial chapters** (e.g., 5 chapters):
   ```bash
   ipen write next book-id --count 5 --words 3000 --context "young protagonist discovering powers"
   ```
...
3. **Review and approve chapters**:
   ```bash
   ipen review list book-id
   ipen review approve-all book-id
   ```

4. **Export the book** (supports txt, md, epub):
   ```bash
   ipen export book-id
   ipen export book-id --format epub
   ```
...
2. **Continue from last chapter**:
   ```bash
   ipen write next book-id --count 3 --words 2500 --context "protagonist faces critical choice"
   ```
   - iPen maintains 7 truth files (world state, character matrix, emotional arcs, etc.) for consistency
...
Use this when you have an existing novel (or partial novel) and want ipen to pick up where it left off.

1. **Import from a single text file** (auto-splits by chapter headings):
   ```bash
   ipen import chapters book-id --from novel.txt
   ```
...
2. **Import from a directory** of separate chapter files:
   ```bash
   ipen import chapters book-id --from ./chapters/
   ```
   - Reads `.md` and `.txt` files in sorted order

3. **Resume interrupted import**:
   ```bash
   ipen import chapters book-id --from novel.txt --resume-from 15
   ```

4. **Continue writing** from the imported chapters:
   ```bash
   ipen write next book-id --count 3
   ```
   - iPen reverse-engineers all 7 truth files from the imported chapters
...
1. **Analyze reference text**:
   ```bash
   ipen style analyze reference_text.txt
   ```
   - Examines vocabulary, sentence structure, tone, pacing

2. **Import style to your book**:
   ```bash
   ipen style import reference_text.txt book-id --name "Author Name"
   ```
...
1. **Import parent canon**:
   ```bash
   ipen import canon spinoff-book-id --from parent-book-id
   ```
...
2. **Continue spinoff**:
   ```bash
   ipen write next spinoff-book-id --count 3 --context "alternate timeline after Chapter 20"
   ```
...
1. **Generate draft only**:
   ```bash
   ipen draft book-id --words 3000 --context "protagonist escapes" --json
   ```

2. **Audit the chapter** (33-dimension quality check):
   ```bash
   ipen audit book-id chapter-1 --json
   ```
...
3. **Revise with specific mode**:
   ```bash
   ipen revise book-id chapter-1 --mode polish --json
   ```
...
```bash
ipen analytics book-id --json
# Shorthand alias
ipen stats book-id --json
```
...
ipen book create --title "The Last Delver" --genre litrpg --chapter-words 3000

# Or set language explicitly
ipen book create --title "My Novel" --genre other --lang en

# Set English as default for all projects
ipen config set-global --lang en
```
...
- Use `ipen genre list` to see all available genres

### Workflow 11: Fan Fiction

```bash
# Create a fanfic from source material
ipen fanfic init --title "My Fanfic" --from source-novel.txt --mode canon

# Modes: canon (faithful), au (alternate universe), ooc (out of character), cp (ship-focused)
ipen fanfic init --title "What If" --from source.txt --mode au --genre other
```
...
## ipen Studio (Web Workbench)

`ipen studio` launches a local web UI (default port 4567) that provides a visual interface for all ipen operations:
...
```bash
ipen studio              # Start on default port 4567
ipen studio -p 8080      # Start on custom port
```
...
```bash
ipen agent "写一部都市题材的小说，主角是一个年轻律师，第一章三千字"
```
...
```bash
ipen write next book-id --count 2 --context "protagonist discovers betrayal, must decide whether to trust mentor"
```
...
```bash
ipen genre list
ipen genre show xuanhuan
```

### Create Custom Genre
```bash
ipen genre create my-genre --name "My Genre"
# Options: --numerical, --power, --era
ipen genre create dark-xuanhuan --name "Dark Xuanhuan" --numerical --power
```

### Copy Built-in Genre for Customization
```bash
ipen genre copy xuanhuan
...
|---------|---------|-------|
| `ipen init [name]` | Initialize project | One-time setup |
| `ipen book create` | Create new book | Returns book-id. `--brief <file>`, `--lang en/zh`, `--genre litrpg/progression/...` |
| `ipen book list` | List all books | Shows IDs, statuses |
| `ipen write next` | Full pipeline (draft→audit→revise) | Primary workflow command |
| `ipen draft` | Generate draft only | No auditing/revision |
| `ipen audit` | 33-dimension quality check | Standalone evaluation |
| `ipen revise` | Revise chapter | Modes: polish/spot-fix/rewrite/rework/anti-detect |
| `ipen agent` | Natural language interface | Flexible requests |
| `ipen style analyze` | Analyze reference text | Extracts style profile |
| `ipen style import` | Apply style to book | Makes style permanent |
| `ipen import canon` | Link spinoff to parent | For prequels/sequels |
| `ipen import chapters` | Import existing chapters | Reverse-engineers truth files for continuation |
| `ipen detect` | AIGC detection | Flags AI-generated passages |
| `ipen export` | Export finished book | Formats: txt, md, epub |
| `ipen analytics` / `ipen stats` | View book statistics | Word count, audit rates, token usage |
| `ipen radar scan` | Platform trend analysis | Informs new book ideas |
| `ipen config set-global` | Configure LLM provider | OpenAI/Anthropic/custom (any OpenAI-compatible) |
| `ipen config set-model <agent> <model>` | Set model override for a specific agent | `--provider`, `--base-url`, `--api-key-env` for multi-provider routing |
| `ipen config show-models` | Show current model routing | View per-agent model assignments |
| `ipen doctor` | Diagnose issues | Check installation |
| `ipen update` | Update to latest version | Self-update |
| `ipen up/down` | Daemon mode | Background processing. Logs to `ipen.log` (JSON Lines). `-q` for quiet mode |
| `ipen review list/approve-all` | Manage chapter approvals | Quality gate |
| `ipen review reject <ch>` | Reject chapter + rollback state | Rolls back to pre-chapter snapshot, discards downstream |
| `ipen write repair-state <ch>` | Rebuild truth files for degraded chapter | Re-runs settler without rewriting prose |
| `ipen fanfic init` | Create fanfic from source material | `--from <file>`, `--mode canon/au/ooc/cp` |
| `ipen genre list` | List all available genres | Shows English and Chinese genres with default language |
| `ipen genre create <id>` | Create custom genre profile | `--name`, `--numerical`, `--power`, `--era` |
| `ipen genre copy <id>` | Copy built-in genre to project | For customization |
| `ipen write rewrite <book> <ch>` | Rewrite a specific chapter | Deletes chapter and later, rewrites from that point |
| `ipen book update [book-id]` | Update book settings | `--chapter-words`, `--target-chapters`, `--status`, `--lang` |
| `ipen book delete <book-id>` | Delete book and all chapters | `--force` to skip confirmation |
| `ipen plan chapter [book-id]` | Generate chapter intent | Preview what next chapter will do before writing |
| `ipen compose chapter [book-id]` | Generate runtime artifacts | Context, rule-stack, trace for next chapter |
| `ipen consolidate [book-id]` | Consolidate chapter summaries | Reduces context for long books (volume-level summaries) |
| `ipen eval [book-id]` | Quality evaluation report | `--json`, `--chapters <range>`. Composite quality score |
| `ipen studio` | Start web workbench | `-p` for port. Local web UI for book management |
| `ipen fanfic show [book-id]` | Display parsed fanfic canon | Shows imported source material analysis |
| `ipen fanfic refresh [book-id]` | Re-import and regenerate fanfic canon | `--from <file>` for updated source material |
...
**"book-id not found"**
- Verify the ID with `ipen book list`
- Ensure you're in the correct project directory

**"Provider not configured"**
- Run `ipen config set-global` with valid credentials
- Check API key and base URL with `ipen doctor`
...
- Try `ipen revise` with `--mode rewrite`
...
2. **Start with style**: If imitating an author, run `ipen style import` before generation
3. **Import first**: For existing novels, use `ipen import chapters` to bootstrap truth files before continuing
4. **Review regularly**: Use `ipen review` to catch issues early
5. **Monitor audits**: Check `ipen audit` metrics to understand quality bottlenecks
...
8. **Check analytics**: Use `ipen analytics` to track quality trends over time
9. **Export frequently**: Keep backups with `ipen export`

## Support & Resources

- **Homepage**: https://github.com/Narcooo/ipen
- **Configuration**: Stored in project root after `ipen init`
- **Truth files**: Located in `books/<id>/story/` per book, with structured JSON in `story/state/`
- **Logs**: Check output of `ipen doctor` for troubleshooting
